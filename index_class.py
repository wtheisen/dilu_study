...
class Index:
    ...
    def query_index(self, image_list, query_feature_list=None, recall=1000, gpu=False):
        ...
        D, I = self.trained_index.search(self.preproc.apply_py(np.array(query_feature_list)), recall)
    ...
    def read_index(self, lean=False):
        ...
        try:
            self.coarse_quantizer = np.load(self.coarse_quant_file + '.npy', allow_pickle=True)
        except:
            self.coarse_quantizer = None
    ...
    def write_index(self):
        ...
        if self.coarse_quantizer: np.save(self.coarse_quant_file, self.coarse_quantizer)
