...
class PreFeatureExtraction:
    ...
    def process_image(self, img_path, flip=False, downsize=False, tfcores=1):
        # image = self.deserialize_image(image_data, resource_path = resource_path).astype(np.uint8)
        # image = raw_imread(img_path).postprocess()
        image = cv2.imread(img_path)
        # print('[LOG]: Image read')
        ...
        try:
            features_struct = feature_extractor.feature_detection_and_description(img_path, self.det_type,
                    self.desc_type, self.kmax, image)
            ...
            feature_resource = Resource(os.path.basename(img_path), self.serialize_features(features, w=image.shape[1], h=image.shape[0]), 'application/octet-stream')

            return self.create_output(feature_resource)

        except Exception as e:
            print(f"[ERROR]: Feature extraction failure for {img_path}\n\t[ERROR DETAILS]: {e}")
    ...
    def create_alg_output(self):
        return  {'name': self.alg_name.replace(" ", ""), 'version': self.alg_num.replace(" ", "")}

    def create_feature_output(self, feature_resource):
